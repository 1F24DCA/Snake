...
package kr.pe.firstfloor.annotation.processor;
...
public class CompileProcessor extends AbstractProcessor {
    private static HashMap<Element, JavaFileObject> classes = new HashMap<>();
    private static StandardJavaFileManager javaFileManager = ToolProvider.getSystemJavaCompiler().getStandardFileManager(new DiagnosticCollector<>(), null, null);
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        TypeElement annotationElement = processingEnv.getElementUtils().getTypeElement("kr.pe.firstfloor.annotation.ExtendableSingletonClass");
                ...
                for (Element enclosedElement : extendedElement.getEnclosedElements()) {
                    ...
                    && enclosedElement.toString().startsWith(extendedElement.getSimpleName().toString()))
                        if (!enclosedElement.getModifiers().contains(Modifier.PROTECTED))
                            processingEnv.getMessager().printMessage(Kind.ERROR, "@ProtectedConstructor "
        ...
        annotationElement = processingEnv.getElementUtils().getTypeElement("kr.pe.firstfloor.annotation.Constant");
        if (annotationElement != null) {
            Set<? extends Element> annotatedElements = roundEnv.getElementsAnnotatedWith(annotationElement);
